...
const SpotDetails = () => {
    ...
    if(spot){
        ...
        let ratingText = "New"
        if(avgStarRating){
            ratingText = `${Number(avgStarRating).toFixed(1)}`
        }
            ...
            <div className="details-container" id={`spot-details-${spotId}`}>
                ...
                <div className="info-container">
                    ...
                    <div className="reserve-container">
                        <div className="rating-container small">
                            ...
                            <h3 className="price-reserve">{`$${price} `}</h3><p className="night">night</p>
                            </div>
...

                            {
                                numReviews>0 &&
                                <>
                                <i class="fa-solid fa-circle" style={{fontSize:"3px"}}></i>
                                <h3 className="num-reviews small">{`${numReviews} ${numReviews>1?"reviews":"review"}`}</h3>
                                </>
                            }
                            </div>
                ...
                <div className="reviews-container">
                    <div className="right-side">
                            ...
                            <div className="avg-rating-container small big">
                            <i className="fa-solid fa-star rating small big"></i>
                            <h3 className="avg-rating small big">{ratingText}</h3>
                            </div>

                            {
                                numReviews>0 &&
                                <>
                                <i class="fa-solid fa-circle big" style={{fontSize:"3px"}}></i>
                                <h3 className="num-reviews small big">{`${numReviews} ${numReviews>1?"reviews":"review"}`}</h3>
                                </>
                            }
                            </div>
                    {
                        user && user.id && Owner.id !== user.id && !reviewsArr.length && <h3 className="first-review">Be the first to post a review!</h3>
                    }
                    {
